...
test('decoded schemes select their scheme handlers', (t) => {
  ...
  // The handler is picked from the decoded scheme, so wsParse really runs.
  const encodedWs = fastURI.parse('w%73://example.com/chat')
  t.equal(encodedWs.scheme, 'ws', 'encoded ws scheme decodes to ws')
  t.equal(encodedWs.resourceName, '/chat', 'the ws handler parsed the resource name')
  t.equal(encodedWs.secure, false, 'the ws handler set the secure flag')
  t.end()
})
